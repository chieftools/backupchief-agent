...
func testRunner(t *testing.T) Runner {
	...
	if runtime.GOOS != "linux" {
		runner.DevelopmentBinary = os.Getenv("RESTIC_DEVELOPMENT_BINARY")
		if runner.DevelopmentBinary == "" {
...
func TestCancellationAndOutputLimits(t *testing.T) {
	for _, mode := range []string{"wait", "overflow"} {
		t.Run(mode, func(t *testing.T) {
			...
			if mode == "overflow" && (!result.Truncated || result.Output != "") {
...
func TestProcessFixture(t *testing.T) {
	...
	case "overflow":
		for i := 0; i < 20000; i++ {
			_, _ = os.Stdout.WriteString(strings.Repeat("synthetic-secret", 100))
		}
		os.Exit(0)
	}
}
