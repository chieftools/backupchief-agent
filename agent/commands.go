...
func (daemon *daemon) receiveCommand(command AgentCommand) error {
	...
	daemon.journal.Commands[command.ID] = &JournalCommand{
		Command: command, RunID: runID, RunKind: runKind, ConfigRevision: command.Payload.RequiredConfigRevision,
		Trigger: "manual", ReceivedAt: protocolTimestamp(daemon.now()), State: "received", Events: []AgentEvent{},
	}
...
func (daemon *daemon) finishWithoutExecution(commandID, status, resultCode, summary string) error {
	...
	journaled.Result = &CommandResult{
		Generation: daemon.bootstrap.Generation, RunID: journaled.RunID, JobID: journaled.Command.Payload.JobID,
		RunKind: journaled.RunKind, Status: status, ResultCode: resultCode,
		StartedAt: now, FinishedAt: now, SnapshotIDs: []string{}, Summary: summary,
	}
	journaled.Sequence++
	if eventID, err := newULID(daemon.now()); err == nil {
		journaled.Events = append(journaled.Events, daemon.eventEnvelope(
...
func (daemon *daemon) flushCommand(ctx context.Context, commandID string) error {
	...
	if logID != "" && !logCompleted {
		...
		if current != nil {
			completion = LogCompletion{
				Generation: daemon.bootstrap.Generation, TotalBytes: current.LogBytes, SHA256: current.LogSHA256,
				ChunkCount: current.NextLogChunk, Truncated: current.LogTruncated, DroppedBytes: current.LogDroppedBytes,
			}
		}
		daemon.mu.Unlock()
		if current != nil {
...
func (daemon *daemon) reconcileOne(ctx context.Context, commandID string) error {
	...
	if journaled.RunKind == "backup" {
		request := restic.Request{
			...
			Password: job.Repository.ServicePassword, Host: daemon.bootstrap.ServerID,
			Tags:           []string{"backupchief-job:" + job.ID, "backupchief-run:" + journaled.RunID},
			TimeoutSeconds: 300, LockWaitSeconds: 30,
		}
...
func (daemon *daemon) reconciledMaintenance(ctx context.Context, command *JournalCommand, job Job) CommandResult {
	...
	result := CommandResult{
		Generation: daemon.bootstrap.Generation, RunID: command.RunID, JobID: job.ID, RunKind: command.RunKind,
		Status: "unresolved", ResultCode: "outcome_unresolved", StartedAt: command.ReceivedAt,
		FinishedAt: timestamp, SnapshotIDs: []string{}, Summary: "The interrupted maintenance outcome could not be proved and was not rerun.",
	}
...
func reconciledResult(generation uint64, command *JournalCommand, result restic.Result, now time.Time) CommandResult {
	...
	reconciled := CommandResult{
		Generation: generation, RunID: command.RunID, JobID: command.Command.Payload.JobID, RunKind: "backup",
		Status: "unresolved", ResultCode: "outcome_unresolved", StartedAt: command.ReceivedAt,
		FinishedAt: timestamp, SnapshotIDs: []string{}, Summary: "The interrupted backup outcome could not be proved and was not rerun.",
	}
	...
	statistics := RunStatistics{
		"files_new": last.Summary.FilesNew, "files_changed": last.Summary.FilesChanged, "files_unmodified": last.Summary.FilesUnmodified,
		"directories_new": last.Summary.DirectoriesNew, "directories_changed": last.Summary.DirectoriesChanged,
		"directories_unmodified": last.Summary.DirectoriesUnmodified, "source_files": last.Summary.SourceFiles,
		"source_bytes": last.Summary.SourceBytes, "stored_bytes": last.Summary.StoredBytes,
	}
