...
func (daemon *daemon) scheduleBackups(ctx context.Context) error {
	...
	for _, job := range config.Jobs {
		...
		operations = append(operations, scheduledOperation{Job: job, RunKind: "backup", Expression: job.Schedule.Expression, Priority: 0})
		for _, maintenance := range []scheduledOperation{
			{Job: job, RunKind: "forget", Expression: job.Retention.ForgetCron, Priority: 1},
			{Job: job, RunKind: "prune", Expression: job.Retention.PruneCron, Priority: 2},
			{Job: job, RunKind: "check_metadata", Expression: job.Integrity.MetadataCron, Priority: 3},
			{Job: job, RunKind: "check_data", Expression: job.Integrity.DataCron, Priority: 4},
		} {
