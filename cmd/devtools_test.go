...
func (controlPlane *developmentControlPlane) enroll(response http.ResponseWriter, request *http.Request) {
	...
	_, _ = fmt.Fprintf(response, `{"protocol_revision":%q,"server_id":"%s","generation":1,"enrolled_at":"2026-09-09T08:15:00.000000Z","config_revision":1}`, agent.ProtocolRevision, developmentTestServerID)
}
...
func developmentConfig(revision uint64) []byte {
	return []byte(fmt.Sprintf(
		`{"metadata":{"protocol_revision":%q,"generation":1,"revision":%d,"schema_version":1,"issued_at":"2026-09-09T08:15:00.000000Z"},"host":{"id":"server_01k4p4f7m1r9d3t6v8w2x5y7za"},"destinations":{},"jobs":{}}`,
		agent.ProtocolRevision,
		revision,
	))
}
