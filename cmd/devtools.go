...
func newDevelopmentRunCommand(version string, stateDirectory *string) *cobra.Command {
	...
	resticBinary := os.Getenv("RESTIC_DEVELOPMENT_BINARY")
	command := &cobra.Command{
		Use:   "run",
	...
	command.Flags().StringVar(&resticBinary, "restic-binary", resticBinary, "Verified local restic executable (required outside Linux)")
	return command
}
