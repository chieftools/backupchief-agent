...
- Continued dispatching queued commands when an earlier command fails, so new backups can be acknowledged and started while the failed command remains available for retry.
- Skipped scheduled operations whose saved job configuration is permanently invalid, and discarded expired unacknowledged manual commands without delaying newer work.
- Logged command dispatch failures and retry delays without logging command payloads or API error details.
