...
func NewSystemFileStore(paths Paths) (*FileStore, error) {
	...
	return &FileStore{
		Paths: paths, ServiceUID: uid, ServiceGID: gid, bootstrapMode: 0o640,
		spoolLimit: SpoolBytesLimit, reserveBytes: SpoolReserveBytes,
	}, nil
}

func NewUserFileStore(paths Paths) *FileStore {
	return &FileStore{
		Paths: paths, ServiceUID: -1, ServiceGID: -1, bootstrapMode: 0o600,
		spoolLimit: SpoolBytesLimit, reserveBytes: SpoolReserveBytes,
	}
}

func NewTestFileStore(paths Paths) *FileStore {
	return &FileStore{Paths: paths, ServiceUID: -1, ServiceGID: -1, spoolLimit: SpoolBytesLimit}
}
