...
func TestRealRepositoryMaintenanceAcrossSnapshotBoundaries(t *testing.T) {
	developmentBinary := os.Getenv("RESTIC_DEVELOPMENT_BINARY")
	if runtime.GOOS != "linux" && developmentBinary == "" {
		t.Skip("set RESTIC_DEVELOPMENT_BINARY to the verified macOS artifact")
	}
	state := filepath.Join(t.TempDir(), "state")
	runner := restic.Runner{State: state, DevelopmentBinary: developmentBinary, AllowLocal: true}
	...
	binary, err := restic.Binary(context.Background(), state, developmentBinary)
	if err != nil {
		t.Fatal(err)
	}
