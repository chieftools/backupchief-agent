...
func TestReenrollAdvancesGenerationAndRetiresOldWork(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/agent/v1/enroll":
			...
			_, _ = io.WriteString(response, `{"protocol_revision":"1.0.0","server_id":"01k4p4f7m1r9d3t6v8w2x5y7za","generation":2,"enrolled_at":"2026-09-11T10:30:00.000000Z","config_revision":1}`)
...
func TestReenrollReplaysIdentityAfterResponseLoss(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case "/agent/v1/enroll":
			...
			response.WriteHeader(http.StatusCreated)
			_, _ = io.WriteString(response, `{"protocol_revision":"1.0.0","server_id":"01k4p4f7m1r9d3t6v8w2x5y7za","generation":2,"enrolled_at":"2026-09-11T11:30:00.000000Z","config_revision":1}`)
		case "/agent/v1/config":
...
func TestRetiredReporterUsesOnlyTerminalEndpoints(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		case request.URL.Path == "/agent/v1/events":
			_, _ = io.WriteString(response, `{"protocol_revision":"1.0.0","results":[{"id":"01k4p4f7m1r9d3t6v8w2x5y7ze","status":"accepted"}]}`)
	...
	runtime := &daemon{
		...
		client:  NewClient(server.URL+"/agent/v1", current.Credential, "1.0.0", server.Client()),
...
func TestRetiredReporterDropsOnlyTheUnknownRun(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		...
		_, _ = io.WriteString(response, `{"protocol_revision":"1.0.0","results":[{"id":"01k4p4f7m1r9d3t6v8w2x5y7zg","status":"accepted"}]}`)
	}))
	defer server.Close()
	if err := store.saveRetiredEnrollments("", []retiredEnrollment{{
		Endpoint: server.URL + "/agent/v1", ServerID: current.ServerID, Generation: 1,
