...
func Run(ctx context.Context, options RunOptions) error {
	...
	if options.HeartbeatEvery == 0 {
		options.HeartbeatEvery = time.Minute
	}
	if options.ConfigEvery == 0 {
		options.ConfigEvery = 10 * time.Minute
	}
	if options.CommandEvery == 0 {
		options.CommandEvery = 10 * time.Second
	}
	if options.ScheduleEvery == 0 {
		options.ScheduleEvery = time.Second
	}
	if options.ReporterEvery == 0 {
		options.ReporterEvery = options.CommandEvery
	}
	if options.DispatchEvery == 0 {
		options.DispatchEvery = options.CommandEvery
	}
	...
	if executor == nil {
		executor = restic.Runner{State: options.Store.stateDirectory(), DevelopmentBinary: options.ResticBinary, AllowLocal: options.AllowLocal}
	}
	...
	go func() {
		errorsChannel <- runAgentLoop(runContext, options.HeartbeatEvery, 0.10, options.Jitter, runtime.sendHeartbeat)
	}()
	go func() {
		errorsChannel <- runAgentLoop(runContext, options.ConfigEvery, 0.10, options.Jitter, runtime.refreshConfig)
	}()
	go func() {
		errorsChannel <- runAgentLoop(runContext, time.Second, 0, options.Jitter, runtime.reloadLocalConfig)
	}()
	go func() {
		errorsChannel <- runAgentLoop(runContext, options.CommandEvery, 0.20, options.Jitter, runtime.processCommands)
	}()
	go func() {
		errorsChannel <- runAgentLoop(runContext, options.ScheduleEvery, 0, options.Jitter, runtime.scheduleBackups)
	}()
	go func() {
		errorsChannel <- runAgentLoop(runContext, options.DispatchEvery, 0.20, options.Jitter, runtime.dispatchCommands)
	}()
	go func() {
...
func runStandalone(ctx context.Context, options RunOptions) error {
	...
	executor := options.Executor
	if executor == nil {
		executor = restic.Runner{State: options.Store.stateDirectory(), DevelopmentBinary: options.ResticBinary, AllowLocal: true}
	}
