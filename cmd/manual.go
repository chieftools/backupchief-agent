...
func newRepositoryCommand(configPath *string) *cobra.Command {
	command := &cobra.Command{Use: "repository", Short: "Manage configured repositories", Args: cobra.NoArgs}
	command.AddCommand(&cobra.Command{
		Use: "init <job>", Short: "Initialize a standalone job repository", Args: cobra.ExactArgs(1),
		RunE: func(command *cobra.Command, args []string) error {
			config, _, err := loadCommandConfiguration(*configPath)
			if err != nil {
				return err
...
func runDirectRestic(command *cobra.Command, configPath string, request restic.Request) error {
	...
	result := (restic.Runner{State: state, AllowLocal: true}).Run(command.Context(), request)
	if result.Output != "" {
