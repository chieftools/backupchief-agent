...
func TestConfigValidateChecksTheSelectedStandaloneFile(t *testing.T) {
  ...
  "jobs":{"job_media":{"type":"file","source":{"root":"/srv/synthetic-media"},"repository":{"destination":"storage_local","path":"media/repository","password":"synthetic-password"},"schedule":"15 4 * * *"}}
}`)
	...
	if output.String() != "Configuration is valid (standalone, 1 jobs).\n" {
		t.Fatalf("unexpected validation output: %q", output.String())
	}

	if err := os.WriteFile(path, []byte(`{"metadata":{"schema_version":1},"destinations":{},"jobs":{},"extra":true}`), 0o600); err != nil {
		t.Fatal(err)
	}
	root = NewRootCommand("1.2.3-test")
	root.SetArgs([]string{"--config", path, "config", "validate"})
	if err := root.Execute(); err == nil || !strings.Contains(err.Error(), "unknown field") {
		t.Fatalf("unexpected invalid configuration result: %v", err)
	}
}
