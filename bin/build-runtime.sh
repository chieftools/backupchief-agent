...
RESTIC_HASH="$(awk '$2 == "darwin-arm64" { print $1 }' "${REPOSITORY_DIR}/restic/BINARY_SHA256SUMS")"
...
bzip2 -dc "restic/restic_darwin_arm64.bz2" \
    > "${STAGING}/restic-${RESTIC_VERSION}-darwin-arm64"
chmod 0755 "${STAGING}/restic-${RESTIC_VERSION}-darwin-arm64"

ACTUAL_RESTIC_HASH="$(shasum -a 256 "${STAGING}/restic-${RESTIC_VERSION}-darwin-arm64" | awk '{print $1}')"
if [ "${ACTUAL_RESTIC_HASH}" != "${RESTIC_HASH}" ]; then
    echo "Unexpected Darwin Restic executable checksum." >&2
    exit 1
fi
...
DARWIN_RESTIC="restic-${RESTIC_VERSION}-darwin-arm64"
...
cat > "${STAGING}/manifest.json" << EOF
{
    ...
    "platforms": {
        "darwin-arm64": {
            ...
            "restic": $(artifact_json "${DARWIN_RESTIC}")
        },
        "linux-amd64": {
